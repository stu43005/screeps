...
Creep.prototype.findCreepWhichCanTransfer = function(creeps) {
  for (let i = 0; i < creeps.length; i++) {
    ...
    if (otherCreep.memory.role === 'carry' && otherCreep.memory.base === this.memory.base) {
      return this.checkEnergyTransfer(otherCreep.carry.energy);
    }
    continue;
  }
  return false;
...
Creep.prototype.pickupWhileMoving = function(reverse) {
  ...
  if (this.room.name === this.memory.routing.targetRoom) {
    ...
    if (containers.length > 0) {
      ...
      return this.checkEnergyTransfer(containers[0].store.energy);
    }
  }
  return reverse;
};
...
Creep.prototype.transferToCreep = function(direction) {
  ...
  for (let i = 0; i < creeps.length; i++) {
    ...
    const return_code = this.transfer(otherCreep, RESOURCE_ENERGY);
    if (return_code === OK) {
      if (config.debug.resources) {
...
const isStore = function(object) {
  const structureTypes = [STRUCTURE_STORAGE, STRUCTURE_TERMINAL, STRUCTURE_CONTAINER];
  if (structureTypes.indexOf(object.structureType) >= 0) {
    return true;
  }
  return false;
