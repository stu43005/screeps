...
Room.prototype.setup = function() {
  ...
  const minerals = this.find(FIND_MINERALS);
  let mineralPath = _.filter(pathsController, path => path.name === 'pathStart-' + minerals[0].id);
  if (mineralPath.length > 0) {
    mineralPath = mineralPath[0];
  } else {
    mineralPath = pathsController[4];
  }
  const pathLB = this.getMemoryPath(mineralPath.name);
  const pathL = this.setLabsTerminal(pathLB);
